...
app.get("/insertData", (req, res) => {
  let sql =
    "INSERT INTO user (id,fullname,password,type) values (2,'Ruchi','123456','customer')";
...
app.post("/complaint", urlencodedParser, (req, res) => {
  ...
  db.query(
    "SELECT id FROM user where fullname= ?",
    [data.name],
    (err, result) => {
      if (!err) {
        idsql = result;
  ...
  var sql =
    "INSERT INTO complaints (user_id,name,complaint,solution) VALUES (?,?,?,?)";

  db.query(
    sql,
    [idsql, data.name, data.complaint, data.solution],
    (err, result) => {
      if (!err) {
        console.log(result);
